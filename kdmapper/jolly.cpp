...
uint64_t jolly::AllocMdlMemory(HANDLE iqvw64e_device_handle, uint64_t size, uint64_t* mdlPtr) {
	/*added by psec*/
...
uint64_t jolly::AllocIndependentPages(HANDLE device_handle, uint32_t size)
{
...
uint64_t jolly::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode, uintptr_t* baseAddress) {
	...
	// Set Module Address for future cleanup
	if (baseAddress != nullptr)
	{
		*baseAddress = kernel_image_base;
	}
...
void jolly::RelocateImageByDelta(portable_executable::vec_relocs relocs, const uint64_t delta) {
...
bool jolly::FixSecurityCookie(void* local_image, uint64_t kernel_image_base)
{
...
bool jolly::ResolveImports(HANDLE iqvw64e_device_handle, portable_executable::vec_imports imports) {
